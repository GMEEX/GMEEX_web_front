    ...
    <div class="content-wrap">
        <div class="container chat-in-box" id="List">
            <p style="padding: 10px 0 10px 20px;font-size: 16px;">
              <router-link to="/uc/order" style="color:#f0a70a;">{{$t('otc.myorder')}}</router-link> ><span style="font-size:14px;">Overview</span>
              </p>
            <Row class="chat-in">
                <Col span="4">
...
export default {
  ...
  methods: {
    ...
    sendOrderStatusNotice: function(type) {
      ...
      var content = "";
      if (type == 1) content = "The other party has paid, please check and confirm receipt! After confirmation, the other party will receive the encrypted assets.";
      else if (type == 3) content = "The other party has cancelled the order!";
      else if (type == 4) content = "The other party is appealing the order, and GMEEX customer service is intervening in the investigation.";
      else if (type == 5) content = "The other party has confirmed the receipt, please check that the encrypted currency has arrived.";
      var jsonParam = {
    ...
    appearOrder: function() {
      ...
      if (parseInt((nowTime - payTime) / 1000) < 1800) {
        //付款时间小于30分钟不允许申诉
        this.$Message.info("Appeal is allowed 30 minutes after payment is completed.");
        return;
      } else {
        this.modal4 = true;
      }
