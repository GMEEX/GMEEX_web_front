        ...
        <div class="send-box">
          ...
          <div class="formbox send-form">
            <Form ref="form" :model="form" :rules="ruleValidate" :label-width="90">
              ...
              <router-link to="/uc/account" style="padding-left: 90px;color:#03ca9b;">{{$t('otc.publishad.tip2')}}</router-link>
              ...
              <FormItem>
                <Button style="background:#03ca9b;color:#fff;border:1px solid #03ca9b;" long @click="handleSubmit('form')" :disabled="disAllowBtn">{{$t('otc.publishad.submit')}}</Button>
    ...
    getMember() {
      ...
      this.$http.get(this.host + this.api.uc.identification).then(res => {
        ...
        if (certifiedBusinessStatus == 2) {
          ...
          this.$Message.warning("Please complete merchant verification");
