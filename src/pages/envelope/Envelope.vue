<template>
  <div class="envelope">
    <div v-if="hasInviteUser" style="width:80%;height: 35px;padding: 5px 0 5px 0px;border-radius: 35px;background-color:rgb(157, 0, 0);margin-left:10%;text-align:center;display: flex;flex-direction:row;justify-content:center;margin-bottom:10px;">
      ...
      <div style="height: 30px;line-height:30px;color: #EEE;">{{inviteUserId}} Please collect the reward！</div>
    </div>
    ...
    <p style="margin-top: -80px;margin-bottom: 80px;text-align:center;color: rgb(255, 136, 79);font-size:13px;">The red envelope is provided through GMEEX (GMEEX.COM).</p>
    <div class="input-panel"  v-if="!hasReceived &&  envelopeInfo.state == 0">
      ...
      <Form ref="formInline" inline>
        <FormItem prop="user">
          <Input type="text" v-model="formInline.phone" placeholder="Please enter the phone number "></Input>
        </FormItem>
        ...
        <FormItem>
          <Button class="register_btn" @click="handleSubmit()">Receive award </Button>
        </FormItem>
      </Form>
    </div>
    ...
    <div class="envelope-result" v-if="hasReceived &&  envelopeInfo.state == 0">
      ...
      <Button v-if="envelopeInfo.invite == 1" class="register_btn" @click="inviteMore()">Invite friends to increase the number of times </Button>
      <p v-if="envelopeInfo.invite ==1" style="font-size:14px;text-align:center;color: #999;margin-top:15px;">Invite a friend to increase the chance to receive it </p>
      <p style="text-align:center;">
        <router-link style="font-size:14px;text-align:center;color: #085fff;margin-top:15px;text-decoration:underline;" to="/app">Download APP | View assets </router-link>
      </p>
    </div>
    <div class="envelope-result" v-if="envelopeInfo.state == 1">
      <p style="font-size:20px;text-align:center;color: #999;margin-top: 5px;">The reward has been collected! </p>
    </div>
    <div class="envelope-result" v-if="envelopeInfo.state == 2">
      <p style="font-size:20px;text-align:center;color: #999;margin-top: 5px;">Reward code has expired ！</p>
    </div>

    <div class="record-list">
      <div class="title">Collection record </div>

      <div class="content">
        <p v-if="envelopeDetailList.length == 0" style="color: #999;margin: 20px 0;">No records </p>
    ...
    <router-link style="font-size:14px;text-align:center;color: #EEE;margin-top:15px;text-decoration:underline;" to="/app">© GMEEX.COM | Download APP</router-link>
    </p>
...
export default {
  data() {
    return {
      spinShow: false,
      hasReceived: false,
      inviteLink: "https://www.gmeex.com/envelope/",
      envelopeNo: "",
      country: "",
      ...
      hasInviteUser: false,
      inviteUserId: "***********",
      inviteUserAvatar: "https://coinexchange.oss-cn-hangzhou.aliyuncs.com/defaultavatar.png",
      envelopeInfo: {
        id: 0,
        name: "**********",
        totalAmount: "0.00",
        ...
        invite: 0,
        type: 0,
        logo: "https://coinexchange.oss-cn-hangzhou.aliyuncs.com/applogo.png",
        bgImage: "https://coinexchange.oss-cn-hangzhou.aliyuncs.com/redenvelope.png",
        state: 0,
        detail: "留给未来一个暴富的可能"
      },
      formInline: {
  ...
  methods: {
    ...
    sendCode(){
      ...
      if(!reg.test(this.formInline.phone)) {
        ...
        if(this.envelopeInfo.id == 0 || this.envelopeInfo.id == null || this.envelopeInfo.id == undefined){
          this.$Message.error("Reward code does not exist ");
          return;
        }
    ...
    inviteMore(){
      ...
      this.$Modal.confirm({
          title: 'Refresh',
          content: '<p>Click "OK" to generate your exclusive red envelope invitation page. </p><br><p>How to use: After entering the exclusive invitation page, share the page to your social friends </p>',
          onOk: () => {
    ...
    getEnvelope(){
        ...
        this.$http.post(this.host + "/uc/redenvelope/query", param).then(res => {
          ...
          if (res.status == 200 && res.body.code == 0) {
            ...
            window.document.title = "【" + this.envelopeInfo.totalAmount + " " + this.envelopeInfo.unit + "】" + this.envelopeInfo.name + "  — GMEEX Digital Assets Exchange"
            this.getEnvelopeDetailList();
          } else {
              this.$Message.error(res.body.message);
          }
