    ...
    <div class="content">
      ...
      <div style="width: 80%;margin: 0 auto;margin-bottom: 60px;">
        ...
        <div v-if="certStatus == 6" style="width: 500px;margin: 0 auto;text-align: center;">
          <Button type="warning" style="width: 120px;background:#1b262d;border-color:#03ca9b" @click="modal_return=true" long size="large">{{$t("uc.identity.shenagain")}}</Button>
        ...
        <div v-if="certStatus == 7" style="width: 500px;margin: 0 auto;text-align: center;">
          <Button type="warning" style="width: 120px;background:#1b262d;border-color:#03ca9b" @click="modal_read=true" long size="large">{{$t("uc.identity.sheqinggain")}}</Button>
        </div>

        <div v-if="certStatus == 3" style="width: 500px;margin: 0 auto;text-align: center;">
          <Button type="warning" style="width: 120px;background:#1b262d;border-color:#03ca9b" @click="modal_read=true" long size="large">{{$t("uc.identity.shenagain")}}</Button>
          <div class="fail-reason" style="margin-top: 50px;font-size: 16px;">
            <Icon type="md-alert" color="red" size="16" />
        ...
        <div v-else-if="certStatus == 2" style="width: 500px;margin: 0 auto;text-align: center;">
          <Button type="warning" style="width: 120px;background:#1b262d;border-color:#03ca9b" @click="publishAd" long size="large">{{$t('nav.fabu')}}</Button>
      ...
      <div class="ipshang" :class="certStatus != 0 ? 'applying' : '' ">
        ...
        <div v-show="certStatus === 0" style="text-align: center;font-size: 16px;margin-top:50px">
          ...
          <router-link target="_blank" to="/helpdetail?cate=1&id=11&cateTitle=常见问题" class="cur" style="color:#03ca9b">{{$t('uc.identity.agreement')}}</router-link>
        </div>
        <!-- 同意第一步的按钮 -->
        <div v-show="certStatus === 0" class="sq">
          <Button @click="apply" style="background:#03ca9b;color:#fff;outline:none;">{{$t("uc.identity.lijishenqing")}}</Button>
        </div>
      </div>
    ...
    <Modal v-model="modal_read">
      ...
      <div class="apply-note">
        ...
        <div style="text-align: left;padding: 30px 0;">
          ...
          <span>
            <font color="#03ca9b">{{auditText}}</font>{{$t('uc.identity.second.agreec')}}</span>
        </div>
        <Button @click="apply2" long style="font-size: 16px;background:#03ca9b;color:#fff;border:1px solid #03ca9b;">{{$t('uc.identity.second.shenqingchngweishangjia')}}</Button>
      </div>
      <p slot="footer">
      ...
      <div class="apply-content">
        ...
        <Form class="apply-form" :model="apply_form" label-position="top">
          ...
          <FormItem style="margin-top: 20px;">
            <Button style="width:100%;background:#03ca9b;color:#fff;border:1px solid #03ca9b;" type="info" @click="apply3('apply_form')" :disabled="applyBtn">{{$t("uc.identity.lijishenqing")}}</Button>
          </FormItem>
        </Form>
      </div>
      <p slot="footer"></p>
  ...
  methods: {
    ...
    apply3(form) {
      if (this.apply_form.telno == "") {
        ...
        this.$store.state.lang == "English" &&
          this.$Message.error("Please fill in your Phone number");
        return;
      }
      if (this.apply_form.wechat == "") {
        ...
        this.$store.state.lang == "English" &&
          this.$Message.error("Please fill in your Wechat number");
        return;
      }
      if (this.apply_form.qq == "") {
        this.$store.state.lang != "English" &&
          this.$Message.error("请填写Whatapp号");
        this.$store.state.lang == "English" &&
          this.$Message.error("Please fill in your Whatapp number");
        return;
      }
