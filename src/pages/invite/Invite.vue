  ...
  <div class="invite">
    <div class="header">
      ...
      <div class="qr-code" style="background:#FFF;padding: 5px 5px;">
        <qriously :value="myInfo.inviteLink" :size="140" foreground="#79a401" />
      </div>
    ...
    <div class="main">
      <div class="invite-content">
        ...
        <div class="content">
          ...
          <div style="font-size: 12px;padding: 10px;margin-left: 30px;width: 90%;margin-bottom: 10px;border: 1px solid #aaff00;">
      ...
      <div class="promotion-tools">
        ...
        <div class="tools">
          <div class="item" @click="showCardItem">
              <img src="https://bizzan310.oss-us-west-1.aliyuncs.com/assets/%E9%82%80%E8%AF%B7%E5%B7%A5%E5%85%B7%E7%AE%B1_rewards%E8%B5%A0%E9%87%912_PS_650x366.png"></img>
              <p class="title">{{$t('invite.pt_title')}}<span style="color:#aaff00;margin-left:10px;font-size:13px;">{{$t('invite.pt_card_title_tips')}}</span></p>
              <p class="desc">{{$t('invite.pt_desc')}}</p>
          </div>
          <div class="item" @click="useInviteImage">
              <img src="https://bizzan310.oss-us-west-1.aliyuncs.com/assets/%E6%8E%A8%E5%B9%BF%E8%AE%A1%E5%88%92%E5%B7%A5%E5%85%B7%E7%AE%B1_%252_800x450.png"></img>
    ...
    <H1 style="color:#ffffff;text-align:center;margin-top: 100px;font-size:20px;;">{{$t('invite.thanks')}}</H1>

    <Drawer :title="$t('invite.pt_title')" :closable="false" v-model="showCardModal" width="600">
      <div class="ptcard-header">
        <img class="card-img" src="https://bizzan310.oss-us-west-1.aliyuncs.com/assets/%E9%82%80%E8%AF%B7%E5%B7%A5%E5%85%B7%E7%AE%B1_rewards%E8%B5%A0%E9%87%912_PS_650x366.png"></img>
        <div class="desc">
      ...
      <div class="ptcard-info">
        ...
        <p class="detail">{{$t('invite.pt_card_rule1')}}：promotion@gmeex.com。</p>
    ...
    <Drawer :title="promotionTitle" :closable="false" v-model="showPromotionModal" width="350" style="text-align:center;">
      <div style="position:relative;width: 318px;" id="promotionImage" ref="promotionImage">
        ...
        <p style="position:absolute;top: 210px;text-align:center;width: 100%;text-align:center;font-size:26px;color:#000000;font-weight:bold;">{{promotionCode}}</p>
        <p style="position:absolute;top: 250px;text-align:center;width: 100%;text-align:center;">Redeem Code</p>
      </div>
    ...
    <Drawer title="Affiliate poster" :closable="false" v-model="showInviteImageModal" width="350" style="text-align:center;">
      <div style="position:relative;width: 318px;" id="inviteImage" ref="inviteImage">
        ...
        <div class="qr-code" style="background:#FFF;position:absolute;top: 260px;left:105px;border-radius: 5px;height: 100px;">
          <qriously :value="myInfo.inviteLink" :size="100" foreground="#aaff00" />
        </div>
        <p style="position:absolute;top: 375px;text-align:center;width: 100%;text-align:center;">Scan QRcode and register</p>
        <p style="position:absolute;top: 395px;text-align:center;width: 100%;text-align:center;font-size:16px;">Invitation Code：{{inviteCode}}</p>
      </div>
      <p style="text-align:center;font-size:12px;color:#000000;margin-top: 10px;">{{$t('invite.imagetips')}}</p>
...
export default {
  data() {
    ...
    return {
      ...
      myInfo: {
        ...
        extrareward: 0,
        partnerlevel: "-",
        inviteLink: "https://www.gmeex.com/register?code=000000"
      },
      promotionRecordPage: {
        pageNo: 1,
        pageSize: 10
  ...
  methods: {
    ...
    saveInviteImage(){
      this.save("inviteImage", "poster");
      this.saveImageLoading = true;
    },
    saveImage(){
      this.save("promotionImage", "Affiliate poster");
      this.saveImageLoading = true;
    },
...
.invite {
  ...
  background: linear-gradient(200deg, #273640, #273640, #273640);
  padding: 0 12%;
  ...
  .header{
    ...
    height: 180px;
    color: #aaff00;
    background: linear-gradient(200deg, #1b262d, #1b262d, #1b262d);
    ...
    .invite-title{
      ...
      margin-right: 170px;
      padding-bottom: 15px;
      border-bottom: 1px dashed #01a781;
      .my-link-text{
        height: 38px;line-height: 38px;
      }
      .my-link-content{
        background:rgba(0, 0, 0, 0);border-radius: 0px;height: 38px;line-height: 38px;width: 600px;text-align: center;
        box-shadow: 0px 0px 0px #000000 inset;
      }
      .copy-button{
        height:32px;line-height:32px;background:linear-gradient(350deg, #aaff7f, #ebff0c);padding:0 20px;margin-left: 20px;border-radius: 0px;color: #595e6c;font-size: 14px;margin-top:3px;box-shadow: 0px 0px 5px #000000;width:100px;text-align:center;
        cursor: pointer;
        &:hover{
          background:linear-gradient(350deg, #90ca33, #d9e141);
        }
        &:focus{
    ...
    .qr-code{
      ...
      width: 150px;
      border-radius: 0px;
      box-shadow: 0px 0px 5px #00000057 inset;
    }
    .mask{
      width: 100%;
      background: #535353e0;
  ...
  p.headertip{
    font-size:12px;text-align:right;color:rgba(1, 167, 129, 0.5);text-shadow: 0 0 0px #ffffff;
  }
  .main {
    min-height: 300px;
    width: 100%;
      ...
      .content{
        ...
        .percent-table{
          ...
          tr{
            background:#1b262d;
            td{
              border-left: 1px solid #374e5c;
              border-top:1px solid #39505f;
            }
          }
        }
        .rule-update{
          font-size: 12px;text-align:right;color:#aaff00;margin-top: 20px;padding-top: 20px;border-top: 1px solid #EDEDED;
        }
      }
      >h2{
        padding-bottom: 10px;
    ...
    .promotion-tools{
      ...
      .tools{
        ...
        .item{
          ...
          p.desc{
            font-size:12px;padding: 0 5px;color: #ffffff;
          }
          &:hover{
            cursor:pointer;
          }
      ...
      .bottom{
        color:#ffffff;font-size:12px;text-align:right;margin-top: 10px;position:absolute;bottom: 5px;position:absolute;width:96%;bottom: 10px;left:2%;
        border-top: 1px dashed #ffffff;padding-top: 10px;text-align:center;
      }
    }
    .invite-rank{
      margin-top: 60px;
      p.ranktitle{
        text-align:left;font-size:12px;margin-bottom: 20px;color:#aaff00;
      }
      h2{
        padding-bottom: 0px;
        color: #aaff00;
      ...
      .rank-tips{
        text-align:right;color:#ffffff;font-size:12px;margin-top:10px;
      }
      .rank-table{
...
.ptcard-header{
  ...
  .desc{
    height:130px;width:100%;color:#ffffff;padding-left: 20px;
    .title{
...
.ptcard-info{
  padding-left: 40px;color:#282e4f;line-height:25px;padding-right:20px;
  .title{
    color: rgb(85, 85, 85);line-height: 25px;padding: 5px 10px;background: #e2e2e2;border: 1px solid #EDEDED;
  }
  .detail{
    margin-top: 10px;
  }
